...
/**
 * ★機能改善: 指定した日付に複数の着用ログを追加する（二重登録防止）
 * @param {object} payload { itemIds: string[], dateString: string }
 * @returns {object} 処理結果
 */
function addWearLogEntries(payload) {
  const { itemIds, dateString } = payload;
  try {
    if (!itemIds || !Array.isArray(itemIds) || !dateString) {
      throw new Error('アイテムIDと日付が指定されていません。');
    }
    const wearDate = new Date(dateString);
    if (isNaN(wearDate.getTime())) {
        throw new Error('無効な日付形式です。');
    }
    
    // Get existing item IDs for the specified date
    const wearLogLastRow = wearLogSheet.getLastRow();
    const existingItemIdsOnDate = new Set();
    if (wearLogLastRow >= 2) {
      const wearLogData = wearLogSheet.getRange(2, 2, wearLogLastRow - 1, 2).getValues();
      const targetDateString = Utilities.formatDate(wearDate, Session.getScriptTimeZone(), 'yyyy-MM-dd');

      wearLogData.forEach(([itemId, date]) => {
        if (date && Utilities.formatDate(new Date(date), Session.getScriptTimeZone(), 'yyyy-MM-dd') === targetDateString) {
          existingItemIdsOnDate.add(String(itemId));
        }
      });
    }

    // Filter out items that are already logged
    const newItemIds = itemIds.filter(id => !existingItemIdsOnDate.has(String(id)));

    if (newItemIds.length === 0) {
      return { status: 'success', message: '追加する新しいアイテムはありませんでした。' };
    }
    ...
    newItemIds.forEach(itemId => {
      const newRowIndex = wearLogSheet.getLastRow() + 1;
      wearLogSheet.getRange(newRowIndex, 1).setFormula(`=MAX(A$1:A${newRowIndex - 1}) + 1`);
      wearLogSheet.getRange(newRowIndex, 2, 1, 2).setValues([[itemId, wearDate]]);
    });

    return { status: 'success', message: `${newItemIds.length}点の着用を記録しました！` };
  } catch (e) {
    console.error('addWearLogEntries Error: ' + e.stack);
    return { status: 'error', message: '着用記録に失敗しました: ' + e.message };
  }
}
